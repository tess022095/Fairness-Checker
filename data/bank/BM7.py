...
y2_df = pd.get_dummies(y2_df, columns=cat_feat, prefix_sep='=')
...
data1.to_csv("bm7_1_" + str(len(data1.columns)), index=False)
...
priv_class = [1] # ['White', 'Male']
reamining_cat_feat = []
df = df.drop(columns=['nr.employed', 'euribor3m', 'emp.var.rate'])
y2_df = y2_df.drop(columns=['nr.employed', 'cons.price.idx', 'duration'])
seed = 76
...
import xgboost
model_1 = xgboost.XGBClassifier(n_estimators=100,
                              learning_rate=0.08,
                              gamma=0,
                              subsample=0.75,
                              colsample_bytree=1,
...
model_2 = xgboost.XGBClassifier(n_estimators=100,
                              learning_rate=0.08,
                              gamma=0,
                              subsample=0.75,
                              colsample_bytree=1,
