...
import torch
import numpy as np
from src.data.bank.scalers import RandomForestClassifier

targerted_model = RandomForestClassifier(n_estimators=1750, criterion='gini', max_features=0.5, min_samples_split=6,
...
class Optimization():
    ...
    def get_sklearn_models(self, X, Z, Y):
        ...
        Args:
            ...
            Y: numpy array of the non-protected of the target value (must be binary)
        """
        partial = targerted_model.fit(X, Y)
        XZ = np.hstack((X, Z))
        full = targerted_model.fit(XZ, Y)
...
class SHAP():
    ...
    def influence(self, method, X):
        ...
        for i in range(X.shape[1]):
            out[:, i] = (np.abs(Yhat[:, self.first[i]] - Yhat[:, self.second[i]]).mean(0) * np.array(self.weights[i])[:,
                                                                                            None]).sum(0)
        return out.mean(0)
...
class OIM():
    ...
    def __init__(self, X, Z, Y):
        XZ = np.hstack((X, Z))
        self.model = targerted_model.fit(XZ, Y)
        self.Zs = np.unique(Z)
        ...
        res = minimize(self.objective, np.random.random(self.Zs.shape[0]), args=(mixXZ, Y), method='SLSQP',
                       constraints=cons,
                       options={'disp': True})
        self.Ws = res.x
