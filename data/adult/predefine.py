...
def disparate_impact(subset_data_orig_train, prediction, protected_attribute):
    ...
    if (privileged_count / total_privileged_count) != 0:
        return abs(
            math.log((unprivileged_count / total_unprivileged_count) / (privileged_count / total_privileged_count)))
    else:
        return 0.0
...
def average_odds_difference(subset_data_orig_train, prediction, y_test, protected_attribute):
    ...
    FPRu = unprivileged_count1 / total_unprivileged_count1
    FPRp = privileged_count1 / total_privileged_count1
    TPRu = unprivileged_count / total_unprivileged_count
    TPRp = privileged_count / total_privileged_count
    # print(total_unprivileged_count, total_privileged_count, total_unprivileged_count1, total_privileged_count1)
    return (abs(TPRu - TPRp) + abs(FPRu - FPRp)) / 2